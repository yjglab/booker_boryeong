...
export default async function reserveCamp() {
  ...
  const nextMonthSltr = await page.$('button[class="next"]');
  ...
  const staySltr = await page.waitForSelector(
    `#change_stay${process.env.RSV_CAMP_STAY}`
  );
  ...
  for (const row of deckRsvRows) {
    ...
    if (deckRsvRowText.includes(process.env.RSV_TG_CAMP)) {
  ...
  const cusRadioButton = await page.$("#cus_gbn_2");
  await cusRadioButton.click();
